...
export class FilesSearchService {
  ...
  public executeRecentSearch = async (accepts?: string[]): Promise<IRecentFile[]> => {
    try {
      ...
      const recentFilesResult: IRecentFile[] = recentSearchData.PrimaryQueryResult.RelevantResults.Table.Rows.map((row: any) => { return this.parseRecentSearchResult(row.Cells); });
