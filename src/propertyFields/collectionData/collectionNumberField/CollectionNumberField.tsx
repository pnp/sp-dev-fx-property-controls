...
export class CollectionNumberField extends React.Component<
  ICollectionNumberFieldProps,
  ICollectionNumberFieldState
> {
  private async: Async;
  ...
  constructor(props: ICollectionNumberFieldProps) {
    ...
    this.delayedValidate = this.async.debounce(
  ...
  public UNSAFE_componentWillUpdate(
    nextProps: ICollectionNumberFieldProps,
    nextState: ICollectionNumberFieldState
  ): void {
  ...
  public render(): React.ReactElement<ICollectionNumberFieldProps> {
    ...
    return (
      <div
        className={`PropertyFieldCollectionData__panel__number-field ${
          styles.numberField
      ...
      >
        <input
          ...
          onChange={async (ev) =>
            await this.valueChange(this.props.field, ev.target.value)
          }
