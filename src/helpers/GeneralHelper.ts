    ...
    switch (formatType) {
      ...
      case '3':
        placeholdersString = this.getLocalizedCountValue(
          ...
          isFuture ? strings.DateTime.L_RelativeDateTime_XMinutesFutureIntervals : strings.DateTime.L_RelativeDateTime_XMinutesIntervals,
          Number(timeString)
        );
        break;
      case '4':
...
export const getPropertyValue = <T = any>(properties: any, targetProperty: string): T => { // eslint-disable-line @typescript-eslint/no-explicit-any
  if (!properties) {
    return undefined;
  }
